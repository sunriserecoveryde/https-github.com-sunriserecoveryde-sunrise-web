...
function FlagChip({ label }: { label: string }) {
  ...
  return (
    <View style={[styles.flagChip, { backgroundColor: colors.muted }]}>
      <Text style={[styles.flagText, { color: colors.navy }]}>{label}</Text>
    </View>
  );
}
...
function BedCard({ patient }: { patient: Patient }) {
  const colors = useColors();
  ...
  return (
    <View style={[styles.bedCard, { backgroundColor: colors.card, borderLeftColor: ac.border }]}>
      ...
      <View style={styles.bedCardBottom}>
        <View style={styles.scoresRow}>
          {showCows && (
            <View style={[styles.scorePill, { backgroundColor: patient.cows! > 12 ? colors.criticalBg : patient.cows! > 8 ? colors.highBg : colors.moderateBg }]}>
              <Text style={[styles.scoreText, { color: patient.cows! > 12 ? colors.critical : patient.cows! > 8 ? colors.high : colors.moderate }]}>
                COWS {patient.cows}
              </Text>
            </View>
          )}
          {showCiwa && (
            <View style={[styles.scorePill, { backgroundColor: patient.ciwa! > 12 ? colors.criticalBg : patient.ciwa! > 8 ? colors.highBg : colors.moderateBg }]}>
              <Text style={[styles.scoreText, { color: patient.ciwa! > 12 ? colors.critical : patient.ciwa! > 8 ? colors.high : colors.moderate }]}>
                CIWA {patient.ciwa}
              </Text>
            </View>
          )}
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPadding, backgroundColor: colors.navy }]}>
        <View style={styles.headerRow}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Oct 26 · Day Shift</Text>
          </View>
          <RoleToggle />
        </View>
        {/* Stats */}
        <View style={styles.statsRow}>
          ...
          <View style={styles.statDivider} />
          <View style={styles.statItem}>
            <Text style={styles.statValue}>{occupiedCount + availableCount + cleaningCount}</Text>
      ...
      <FlatList
        ...
        keyExtractor={p => p.id}
        renderItem={({ item }) => <BedCard patient={item} />}
...
const styles = StyleSheet.create({
  ...
  footerBeds: { marginTop: 8, paddingHorizontal: 0 },
