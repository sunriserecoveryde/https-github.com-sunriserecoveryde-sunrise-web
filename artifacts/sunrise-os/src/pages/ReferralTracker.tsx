...
import {
  ...
  ArrowRight, BarChart3, MapPin, Phone, Mail, Calendar, X,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-end">
        ...
        <LockedButton locked={readOnly} onClick={() => setNewReferralOpen(true)} className="flex items-center gap-2 bg-sunrise-blue text-white px-4 py-2 rounded font-medium shadow-sm hover:bg-sunrise-blue-light transition-colors text-sm">
      ...
      <div className="bg-gradient-to-r from-sunrise-orange/10 to-transparent border border-sunrise-orange/25 rounded-xl p-5 flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-3 mt-3">
            <button onClick={() => { setOutreachSaved('alert'); setTimeout(() => setOutreachSaved(null), 2500); }} className="text-sm font-bold text-sunrise-orange hover:underline flex items-center gap-1">
              Log Outreach <ArrowRight className="w-4 h-4" />
            </button>
            <button onClick={() => setNewReferralOpen(false)} className="text-sm font-bold text-slate hover:text-navy flex items-center gap-1">
              Dismiss
            </button>
          </div>
        </div>
                          ...
                          <div className="grid grid-cols-3 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <div className="flex gap-2">
                                <LockedButton locked={readOnly} onClick={() => { setOutreachSaved('Outreach logged'); setTimeout(() => setOutreachSaved(null), 2500); }} className="px-3 py-1.5 bg-sunrise-blue text-white text-xs font-semibold rounded hover:bg-sunrise-blue-light">Log Outreach</LockedButton>
        ...
        {activeTab === 'Outreach' && (
          <div className="p-5">
            <div className="flex justify-between items-center mb-4">
              ...
              <LockedButton locked={readOnly} onClick={() => { setOutreachSaved('Outreach logged'); setTimeout(() => setOutreachSaved(null), 2500); }} className="flex items-center gap-2 bg-sunrise-blue text-white px-3 py-2 rounded text-xs font-semibold hover:bg-sunrise-blue-light">
              ...
              {OUTREACH_LOG.map(o => (
                <div key={o.id} className={`border rounded-lg p-4 ${o.outcome === 'At Risk' ? 'bg-red-50 border-red-200' : 'bg-white border-border'}`}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button onClick={() => { setOutreachSaved('outreach-ack'); setTimeout(() => setOutreachSaved(null), 2500); }} className="text-sunrise-blue text-xs font-medium hover:underline flex-none">
      ...
      {newReferralOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setNewReferralOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-[500px]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h2 className="text-lg font-bold text-navy">Add Referral Source</h2>
              <button onClick={() => setNewReferralOpen(false)} className="text-slate hover:text-navy"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-slate uppercase mb-1">Organization Name *</label>
                  <input type="text" className="w-full border border-border rounded-lg px-3 py-2 text-sm" placeholder="e.g. Johns Hopkins Hospital — Social Work Dept." />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate uppercase mb-1">Source Type</label>
                  <select className="w-full border border-border rounded-lg px-3 py-2 text-sm">
                    <option>Hospital / ED</option><option>Physician / PCP</option><option>Behavioral Health Org</option><option>EAP / Employer</option><option>Court / Drug Court</option><option>Self / Family</option><option>Alumni</option><option>Insurance / Managed Care</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate uppercase mb-1">Primary Contact</label>
                  <input type="text" className="w-full border border-border rounded-lg px-3 py-2 text-sm" placeholder="Name, title" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate uppercase mb-1">Phone</label>
                  <input type="tel" className="w-full border border-border rounded-lg px-3 py-2 text-sm" placeholder="(xxx) xxx-xxxx" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate uppercase mb-1">Email</label>
                  <input type="email" className="w-full border border-border rounded-lg px-3 py-2 text-sm" placeholder="contact@org.com" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-slate uppercase mb-1">BD Rep Assigned</label>
                  <select className="w-full border border-border rounded-lg px-3 py-2 text-sm">
                    <option>Unassigned</option><option>Alex Torres (BD Director)</option><option>Monica Reyes (Community Liaison)</option>
                  </select>
                </div>
              </div>
            </div>
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => setNewReferralOpen(false)} className="flex-1 border border-border rounded-xl py-2.5 text-sm text-slate hover:bg-gray-50">Cancel</button>
              <button onClick={() => { setNewReferralOpen(false); setReferralSaved(true); setTimeout(() => setReferralSaved(false), 2500); }} className="flex-1 bg-sunrise-blue text-white rounded-xl py-2.5 text-sm font-semibold">Add Source</button>
            </div>
          </div>
        </div>
      )}
      ...
      {referralSaved && (
        <div className="fixed bottom-6 right-6 bg-green-600 text-white rounded-xl shadow-lg px-5 py-3 text-sm font-semibold flex items-center gap-2 z-50">
          <Network className="w-4 h-4" /> Referral source added
        </div>
      )}
    </div>
  );
